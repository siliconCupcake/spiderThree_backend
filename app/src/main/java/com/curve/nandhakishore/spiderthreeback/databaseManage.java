...
public class databaseManage {
    ...
    private String[] allColumns_W = {W_CITY, W_ICON, W_MAIN, W_DESC, W_TEMP, W_MIN, W_MAX, W_PRESSURE, W_HUMIDITY, W_WIND, W_CLOUDS, W_TIME, W_COUNTRY};
    private String[] allColumns_F = {W_CITY, W_DATE, W_ICON, W_MAIN, W_DESC, W_TEMP, W_MIN, W_MAX, W_PRESSURE, W_HUMIDITY, W_WIND, W_CLOUDS};

    private static final String CREATE_DB_W = "CREATE TABLE " + WEATHER_NAME + "( " + W_CITY + " TEXT PRIMARY KEY, "
    ...
    private static final String CREATE_DB_F = "CREATE TABLE " + FORECAST_NAME + "( " + W_CITY + " TEXT, "
    ...
    public long addForecast (List c, String city){
        ContentValues cv = new ContentValues();
        cv.put(W_CITY, city);
    ...
    public ArrayList<List> getForecast(String city) {
    ...
    public void removeWeather(CurrentWeather c){
        String whereClause = W_CITY + " = ?";
        String[] args = new String[] {c.getName()};
        myDatabase.delete(WEATHER_NAME, whereClause ,args);
    }

    public void removeForecast(ForecastWeather c) {
        String whereClause = W_CITY + " = ?";
        String[] args = new String[] {c.getCity().getName()};
    ...
    private CurrentWeather convertData_W(Cursor cursor) {
        ...
        weather.setIcon(cursor.getString(1));
        weather.setMain(cursor.getString(2));
        weather.setDescription(cursor.getString(3));
        list.add(weather);
        c.setWeather(list);

        Main main = new Main();
        main.setTemp(cursor.getDouble(4));
        main.setTempMin(cursor.getDouble(5));
        main.setTempMax(cursor.getDouble(6));
        main.setPressure(cursor.getDouble(7));
        main.setHumidity(cursor.getInt(8));
        c.setMain(main);

        Wind wind = new Wind();
        wind.setSpeed(cursor.getDouble(9));
        c.setWind(wind);

        Clouds clouds = new Clouds();
        clouds.setAll(cursor.getInt(10));
        c.setClouds(clouds);

        c.setDt(cursor.getInt(11));

        Sys sys = new Sys();
        sys.setCountry(cursor.getString(12));
        c.setSys(sys);
